from PyQt6.QtWidgets import QApplication, QWidget, QMainWindow, QPushButton, QHBoxLayout, QVBoxLayout, QScrollArea, QLabel, QFileDialog, QLineEdit, QSpinBox, QDoubleSpinBox
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # accuracy
        accuracy_label = QLabel("Target Accuracy")
        self.accuracy_input = QLineEdit(f"{NeuralNetworkModel.TARGET_accuracy}") # QDoubleSpinBox()
        # learning rate
        ...
        right_layout.addWidget(accuracy_label)
        right_layout.addWidget(self.accuracy_input)
    ...
    def handleTrain(self):
        try:
            ...
            iterations = None
        try:
            target_accuracy = float(self.accuracy_input.text())
        except Exception:
        ...
        self.worker = ParallelWorker(self.scroll_area, self.training_label, iterations, target_accuracy, learning_rate, self.model)
        self.worker.result_signal.connect(self.trainFinished)
        self.worker.start()
        self.canvas.set_model(self.model)
        self.train_button.setText("Train model and save it as (.pkl)")
    ...
    def handleLoad(self):
        ...
        self.load_button.setText(f"Loading  model '{_}'")
        self.model = NeuralNetworkModel()
        self.model.load_model(file_path=fname)
        self.canvas.set_model(self.model)
        self.load_button.setText("Load model (.pkl)")
        

class ParallelWorker(QThread):
    result_signal = pyqtSignal()

    def __init__(self, scroll_area: QScrollArea, label: QLabel, iterations: int, target_accuracy: float, learning_rate: float, model: NeuralNetworkModel):
        super().__init__()
    ...
    def run(self):
        training = self.model.train(target_accuracy=self.target_accuracy, epochs=self.iterations, learning_rate=self.learning_rate)
        try:
            while True:
                text, epoch, train_accuracy, train_loss, val_accuracy, val_loss = next(training)
